  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(entries => {
      ...
      entries.forEach(element => {
        ...
        const intersectingElemIdx = parseInt(element.target.getAttribute('data-index')) 
      ...
      } else if (belowIndeces.length > 0 && minIndex <= activeIndexRef.current) { 
      ...
      // 85
      threshold: 1.0,
      rootMargin: screenSizeBoolean || isItPhone ? '-100px 0px 0px 0px' : '-40px 0px 0px 0px'
    }
    )
  ...
  useEffect(() => {
    ...
    */
    if(activeSlideRef.current)
    {
      ...
      sideBar.current.scrollTo({left: activeSlideRef.current.offsetLeft - 78, behavior: "smooth"})
    }
  }, [activeIndex])
...
<div class={styles["menupage-container"]}>
      <SideBar sideBar={sideBar} headerRefs={headerRefs} activeIndex={activeIndex} activeSlideRef={activeSlideRef} categoriesRefs={categoriesRefs}/>
      ...
      <div class={styles["menupagebody-filler"]} style={{width: "286px", flexShrink: 100}}></div>
    </div>
  )
}
